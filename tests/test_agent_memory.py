...
def test_session_id_bypasses_cache_and_records_history() -> None:
    import agent

    fake = _fake_agent("first answer")
    with patch.object(agent, "get_agent", return_value=fake):
        r1 = agent.ask("First question?", session_id="s1")
        # Same wording — still must call the agent, session context differs.
        fake.invoke.return_value = {"output": "second answer", "intermediate_steps": []}
        r2 = agent.ask("First question?", session_id="s1")

    assert r1["cached"] is False
    assert r2["cached"] is False
    assert fake.invoke.call_count == 2
    # Second call must prepend prior-turn context.
