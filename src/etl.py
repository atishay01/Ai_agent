...
def profile(df: pd.DataFrame, name: str) -> None:
    log.info(
        "{name}: shape={shape} nulls={nulls} duplicates={dups}",
        name=name,
        shape=df.shape,
...
def build_products() -> pd.DataFrame:
    log.info("EXTRACT products + category translation")
    ...
    for col in dim_cols:
        df[col] = df.groupby("product_category_name")[col].transform(lambda s: s.fillna(s.median()))
...
def build_geolocation() -> pd.DataFrame:
    log.info("EXTRACT geolocation (large ~1M row CSV)")
...
def build_orders() -> pd.DataFrame:
    ...
    return df[["order_id", "customer_id", "order_status", *ts_cols, "delivery_days", "is_late"]]
...
def build_order_payments() -> pd.DataFrame:
    log.info("EXTRACT order_payments")
...
def build_order_reviews() -> pd.DataFrame:
    log.info("EXTRACT order_reviews")
...
def load(engine, name: str, df: pd.DataFrame) -> None:
    log.info("LOAD {name} ({rows:,} rows)", name=name, rows=len(df))
